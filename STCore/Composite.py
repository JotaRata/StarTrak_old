...
def Awake(root, ItemList, TrackedStars):
	...
	loadBar[0].destroy()
	Sidebar = ttk.Frame(CompositeFrame, width = 400)
	...
	ttk.Label(Sidebar, text = "Modo de Imagen:").grid(row = 2, column = 0, sticky = tk.W)
	ttk.OptionMenu(Sidebar, _MODE_,_MODE_.get(), *STCore.Settings.VisModes).grid(row = 2, column = 1, sticky = tk.E)
	ttk.Label(Sidebar, text = "Mapa de color:").grid(row = 3, column = 0, sticky = tk.W)
